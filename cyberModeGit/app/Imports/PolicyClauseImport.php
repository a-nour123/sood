    ...
    WithValidation
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) {
            ...
            $policyNameEn = $row[$this->columnsMapping['policy_name_en'] ?? 'policy_name_en'] ?? null;
            $policyNameAr = $row[$this->columnsMapping['policy_name_ar'] ?? 'policy_name_ar'] ?? null;
            $documentNames = $row[$this->columnsMapping['document_ids']] ?? null;

            // Merge policy_name_en and ar into a JSON array
            $policyName = [
                'en' => $policyNameEn,
                'ar' => $policyNameAr
            ];
            ...
            // Check if the policy already exists (by both en and ar names)
            $centerPolicy = CenterPolicy::where('policy_name->en', $policyNameEn)
                ->orWhere('policy_name->ar', $policyNameAr)
                ->first();
            ...
            if ($centerPolicy) {
                ...
                // Update the policy's document_ids and names if changed
                $centerPolicy->policy_name = $policyName;
    ...
    public function rules(): array
    {
        ...
        $policy_name_en = $this->columnsMapping['policy_name_en'] ?? 'policy_name_en';
        $policy_name_ar = $this->columnsMapping['policy_name_ar'] ?? 'policy_name_ar';
        $document_ids = $this->columnsMapping['document_ids'] ?? 'document_ids';
        return [
            $policy_name_en => ['required', 'max:255'],
            $policy_name_ar => ['required', 'max:255'],
