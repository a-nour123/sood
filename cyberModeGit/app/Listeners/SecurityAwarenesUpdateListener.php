    ...
    public function handle(SecurityAwarenesUpdate $event)
    {
        ...
        foreach ($reviewer as $review) {
            ...
            if ($reviewer_person) {
                ...
                // Handle the case where User::find($review) returns null
                $reviewersNames = null;
                break; // No need to continue the loop if a user is not found
            }
        }
        
        // If $reviewersNames is not null, trim the trailing comma and enclose in parentheses
        if ($reviewersNames !== null) {
