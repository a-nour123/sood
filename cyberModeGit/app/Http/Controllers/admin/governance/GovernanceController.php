...
use App\Models\ObjectiveCommentReader;
...
class GovernanceController extends Controller
{
    ...
    public function index()
    {
        ...
        $pageConfigs = [
            'pageHeader' => false,
            '284Layout' => "content-left-sidebar",
    ...
    public function editControl(Request $request, $id)
    {
        ...
        $html = "";
        foreach ($controls as $control) {
            $description = json_decode($control->description, true) ?? [];
            ...
            //description_en
            $html .= "<div class='mb-1'>
                  <label for='desc' class='form-label'>Description English</label>
                  <textarea   class='form-control'  name='description_en' >$description_en </textarea>
                  <span class='error error-description ' ></span>

                </div>";
            //description_ar
            $html .= "<div class='mb-1'>
                  <label for='desc' class='form-label'>Description Arabic</label>
                  <textarea   class='form-control'  name='description_ar' >$description_ar</textarea>
    ...
    public function storeControl2(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                $control->description = ['en' => $request->description_en, 'ar' =>  $request->description_ar];
    ...
    public function storeAudit(Request $request)
    {
        ...
        foreach ($ListTestIds as $id) {
            ...
            $test = FrameworkControlTest::where('id', $id)->first();
            ...
            try {
                ...
                if ($request->hasFile('file')) {
                    ...
                    if ($request->file('file')->isValid()) {
                        ...
                        // Update document data for change log
                        $documentData['file_id'] = $fileId->id;
                    } else {
                ...
                // Generate change log for new document creation
                $emptyOriginalData = [
                    'document_name' => null,
                    'privacy' => null,
                ...
                $changes = $this->generateChangeLog($emptyOriginalData, $documentData, $uploadedFileName !== null, null, $uploadedFileName);

                $baseMessage = __('locale.New Document Created With Name') . ' "' . $document->document_name . '". '
                    . __('locale.CreatedBy') . ' "' . auth()->user()->name . '". ';
                $logMessage = $baseMessage . ($changes ? 'Initial values: ' . $changes : 'Document created with default values.');

                write_log($document->id, auth()->id(), $logMessage, 'Creating Document');
            ...
            if ($validator->fails()) {
                ...
                try {
                    ...
                    $originalData = [
                        ...
                        'document_reviewer' => $document->document_reviewer,
                        'additional_stakeholders' => $document->additional_stakeholders,
                        'team_ids' => $document->team_ids,
                    ...
                    if ($request->hasFile('file')) {
                        if ($request->file('file')->isValid()) {
                            ...
                            $document->update(['file_id' => $file->id]);
                            $newData['file_id'] = $file->id;
                        } else {
                            DB::rollBack();
                    ...
                    $changes = $this->generateChangeLog($originalData, $newData, $fileChanged, $oldFileName, $newFileName);

                    $baseMessage = __('locale.A Document With Name') . ' "' . $document->document_name . '" ' . __('locale.UpdatedBy') . ' "' . auth()->user()->name . '". ';
                    $logMessage = $baseMessage . ($changes ? 'Changes: ' . $changes : 'No changes detected.');

                    DB::commit();
                    event(new DocumentUpdated($document));

                    // Write detailed log with changes
                    write_log($document->id, auth()->id(), $logMessage, 'Updating Document');

                    $response = array(
                        'status' => true,
                        'reload' => true,
                        'message' => __('governance.DocumentWasUpdatedSuccessfully'),
                    );
                    return response()->json($response, 200);
                } catch (\Throwable $th) {
                    DB::rollBack();
    ...
    /**
     * Generate a detailed change log comparing original and new data
     */
    private function generateChangeLog($originalData, $newData, $fileChanged = false, $oldFileName = null, $newFileName = null)
    {
        $changes = [];

        // Field mappings for better readability in logs
        $fieldLabels = [
            ...
            'control_ids' => 'Control IDs',
            'framework_ids' => 'Framework IDs',
            'document_owner' => 'Document Owner',
            'document_reviewer' => 'Document Reviewer',
            'additional_stakeholders' => 'Additional Stakeholders',
            'team_ids' => 'Team IDs',
            'content' => 'Content',
        ...
        foreach ($newData as $field => $newValue) {
            ...
            switch ($field) {
                case 'document_status':
                    $oldStatus = $statusLabels[$originalValue] ?? $originalValue;
                    $newStatus = $statusLabels[$newValue] ?? $newValue;
                    $changes[] = "{$fieldLabel} changed from '{$oldStatus}' to '{$newStatus}'";
                    break;

                case 'document_owner':
                case 'document_reviewer':
                    $oldUser = $originalValue ? (User::find($originalValue) ? User::find($originalValue)->name : 'Not Set') : 'Not Set';
                    $newUser = $newValue ? (User::find($newValue) ? User::find($newValue)->name : 'Not Set') : 'Not Set';
                    if ($oldUser !== $newUser) {
                        $changes[] = "{$fieldLabel} changed from '{$oldUser}' to '{$newUser}'";
                    }
                    break;

                case 'privacy':
                    $oldPrivacy = $originalValue ? (Privacy::find($originalValue) ? Privacy::find($originalValue)->name : 'Not Set') : 'Not Set';
                    $newPrivacy = $newValue ? (Privacy::find($newValue) ? Privacy::find($newValue)->name : 'Not Set') : 'Not Set';
                    if ($oldPrivacy !== $newPrivacy) {
                        $changes[] = "{$fieldLabel} changed from '{$oldPrivacy}' to '{$newPrivacy}'";
                    }
                    break;

                case 'control_ids':
                    $oldControlsCount = $originalValue ? count(explode(',', $originalValue)) : 0;
                    $newControlsCount = $newValue ? count(explode(',', $newValue)) : 0;
                    $changes[] = "{$fieldLabel} updated ({$oldControlsCount} → {$newControlsCount} controls)";
                    break;

                case 'framework_ids':
                    $oldFrameworksCount = $originalValue ? count(explode(',', $originalValue)) : 0;
                    $newFrameworksCount = $newValue ? count(explode(',', $newValue)) : 0;
                    $changes[] = "{$fieldLabel} updated ({$oldFrameworksCount} → {$newFrameworksCount} frameworks)";
                    break;

                case 'additional_stakeholders':
                    $oldStakeholdersCount = $originalValue ? count(explode(',', $originalValue)) : 0;
                    $newStakeholdersCount = $newValue ? count(explode(',', $newValue)) : 0;
                    $changes[] = "{$fieldLabel} updated ({$oldStakeholdersCount} → {$newStakeholdersCount} stakeholders)";
                    break;

                case 'team_ids':
                    $oldTeamsCount = $originalValue ? count(explode(',', $originalValue)) : 0;
                    $newTeamsCount = $newValue ? count(explode(',', $newValue)) : 0;
                    $changes[] = "{$fieldLabel} updated ({$oldTeamsCount} → {$newTeamsCount} teams)";
                    break;

                case 'content':
                    $oldLength = $originalValue ? strlen($originalValue) : 0;
                    $newLength = $newValue ? strlen($newValue) : 0;
                    $changes[] = "Content updated ({$oldLength} → {$newLength} characters)";
                    break;

                case 'version_name':
                    $oldLength = $originalValue ? strlen($originalValue) : 0;
                    $newLength = $newValue ? strlen($newValue) : 0;
                    $changes[] = "Version name updated ({$oldLength} → {$newLength} characters)";
                    break;

                default:
                    $oldVal = $originalValue ?: 'Not Set';
                    $newVal = $newValue ?: 'Not Set';
                    $changes[] = "{$fieldLabel} changed from '{$oldVal}' to '{$newVal}'";
                    break;
            }
        }
        ...
        if ($fileChanged) {
            if ($oldFileName && $newFileName) {
                $changes[] = "File replaced: '{$oldFileName}' → '{$newFileName}'";
            } elseif ($newFileName) {
                $changes[] = "File uploaded: '{$newFileName}'";
            } elseif ($oldFileName) {
                $changes[] = "File removed: '{$oldFileName}'";
            }
        }
    ...
    {
        $contentChange = Document::where('id', $id)->first()->file ?? null;
    ...
    public function editDocument(Request $request, $id)
    {
        ...
        if ($document) {
            ...
            $data['version_name'] = $document->version_name;
    ...
    public function send_note(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                event(new DocumentCommentCreated($note));
    ...
    public function addObjectiveToControl(Request $request)
    {
        ...
        if ($request->objective_adding_type == 'new') {
            $rules = [
                ...
                'objective_description_en' => ['required', 'max:1000'],
                'objective_description_ar' => ['required', 'max:1000'],
            ...
            try {
                if ($request->objective_adding_type == 'new') {
                    ...
                    $objective = ControlObjective::create([
                        ...
                        'description' => ['en' => $request->objective_description_en, 'ar' =>  $request->objective_description_ar],
                    ]);
    ...
    public function showObjectiveComments($controlControlObjectiveId)
    {
        ...
        $this->makeTheCommentUnreadReaded($controlControlObjectiveId);
    ...
    public function makeTheCommentUnreadReaded($controlControlObjectiveId)
    {
        $userId = auth()->id();

        $comments = ObjectiveComment::where('control_control_objective_id', $controlControlObjectiveId)
            ->where('user_id', '!=', $userId) // Exclude own comments
            ->get();

        foreach ($comments as $comment) {
            ObjectiveCommentReader::updateOrCreate(
                [
    ...
    public function showObjectiveCommentsCount($controlControlObjectiveId)
    {
        $userId = auth()->id();

        // Get the count of comments that have NO read entry for this user or read entry is false
        $unreadCount = ObjectiveComment::where('control_control_objective_id', $controlControlObjectiveId)
            ->where('user_id', '!=', $userId) // exclude user's own comments
    ...
    public function sendObjectiveComment(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                event(new ControlObjectiveCommentCreated($comment));
    ...
    public function downloadDocumentCommentFile($comment_id)
    {
        try {
            $comment = DocumentNoteFile::where('id', $comment_id)->first() ?? null;
            $fileExists = Storage::disk('local')->exists($comment->unique_name);
            if ($comment && $fileExists) {
                $filePath = storage_path('app/' . $comment->unique_name);
    ...
    public function notificationsSettingscontrol()
    {
        ...
        $users = User::select('id', 'name')->get();  // getting all users to list them in select input of users
        $moduleActionsIds = [34, 35, 36, 37, 38, 39, 40, 85, 86, 141];   // defining ids of actions modules
        $moduleActionsIdsAutoNotify = [87, 140];  // defining ids of actions modules
        ...
        $actionsVariables = [
            ...
            140 => ['Control_Owner', 'Control_Name', 'Control_Description', 'Objective', 'Responsible', 'Due_Date'],
            141 => ['Control_Owner', 'Control_Name', 'Control_Description', 'Objective', 'Responsible', 'Due_Date', 'Comment', 'Creator'],
        ...
        $actionsRoles = [
            ...
            140 => ['Control-Owner' => __('locale.ControlOwner'), 'Responsible_Person' => __('locale.Responsible_Person'), 'Control-Tester' => __('locale.ControlTester'), 'Auditer' => __('locale.Auditer')],
            141 => ['Control-Owner' => __('locale.ControlOwner'), 'Responsible_Person' => __('locale.Responsible_Person'), 'Control-Tester' => __('locale.ControlTester'), 'Auditer' => __('locale.Auditer')],
        ];
        // getting actions with their system notifications settings, sms settings and mail settings to list them in tables
        $actionsWithSettings = Action::whereIn('actions.id', $moduleActionsIds)
            ->leftJoin('system_notifications_settings', 'actions.id', '=', 'system_notifications_settings.action_id')
    ...
    public function notificationsSettingsDocumentation()
    {
        ...
        $users = User::select('id', 'name')->get();  // getting all users to list them in select input of users
        $moduleActionsIds = [53, 54, 55, 56, 57, 58, 114, 115, 134, 135, 136, 137, 144];   // defining ids of actions modules
        ...
        $actionsVariables = [
            ...
            136 => ['Name'],
            137 => ['Name'],
            144 => ['note', 'Document_Type', 'Status', 'Document_Name', 'Last_Review_Date', 'Next_Review_Date', 'Controls', 'Teams', 'Stakeholders', 'Frameworks', 'Created_By'],
        ...
        $actionsRoles = [
            ...
            134 => ['Document-Owner' => __('governance.DocumentOwner'), 'Team-teams' => __('governance.TeamsOfDocument'), 'Stakeholder-teams' => __('governance.StakeholderOfDocument'), 'Document-Creator' => __('governance.DocumentCreator'), 'reviewers-teams' => __('governance.ReviewersOfDocument'), 'Control-Owner' => __('governance.ControlOwner')],
            135 => ['Document-Owner' => __('governance.DocumentOwner'), 'Team-teams' => __('governance.TeamsOfDocument'), 'Stakeholder-teams' => __('governance.StakeholderOfDocument'), 'Document-Creator' => __('governance.DocumentCreator'), 'reviewers-teams' => __('governance.ReviewersOfDocument'), 'Control-Owner' => __('governance.ControlOwner')],
            136 => ['Document-Owner' => __('governance.DocumentOwner'), 'Team-teams' => __('governance.TeamsOfDocument'), 'Stakeholder-teams' => __('governance.StakeholderOfDocument'), 'Document-Creator' => __('governance.DocumentCreator'), 'reviewers-teams' => __('governance.ReviewersOfDocument'), 'Control-Owner' => __('governance.ControlOwner')],
            137 => ['Document-Owner' => __('governance.DocumentOwner'), 'Team-teams' => __('governance.TeamsOfDocument'), 'Stakeholder-teams' => __('governance.StakeholderOfDocument'), 'Document-Creator' => __('governance.DocumentCreator'), 'reviewers-teams' => __('governance.ReviewersOfDocument'), 'Control-Owner' => __('governance.ControlOwner')],
            144 => ['Document-Owner' => __('governance.DocumentOwner'), 'Team-teams' => __('governance.TeamsOfDocument'), 'Stakeholder-teams' => __('governance.StakeholderOfDocument'), 'Document-Creator' => __('governance.DocumentCreator'), 'Control-Owner' => __('governance.ControlOwner')],
    ...
    public function getDomainInframework(Request $request)
    {
        ...
        // Fetch only the top-level families with their subfamilies (id + name only)
        $families = Family::whereIn('id', $DomainParentId)
            ->whereNull('parent_id')
            ->select('id', 'name') // ✅ select only necessary columns
            ->with([
                'families:id,name,parent_id' // ✅ eager-load only what’s needed
            ])
            ->get();

        // ✅ Transform data to ensure only name/id/families are returned
        $formatted = $families->map(function ($family) {
            return [
                'id' => $family->id,
                'name' => $family->name,
                'families' => $family->families->map(function ($sub) {
                    return [
                        'id' => $sub->id,
                        'name' => $sub->name,
                    ];
