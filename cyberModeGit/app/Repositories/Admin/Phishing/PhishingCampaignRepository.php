...
{
    use PhishingMailTrait,CertificateGenerationTrait;
    public function index()
    {
        ...
        $trainingModules = LMSTrainingModule::with('level.course')->withoutTrashed()->where('training_type', 'campaign')->orderBy('created_at', 'desc')->get();
                ...
                if ($campaign->delivery_type === 'immediatly') {
                    ...
                    foreach ($campaign->emailTemplates as $mail) {
                        foreach ($campaign->employees as $employee) {
                            ...
                            $isSent = $this->sendPhishingMail2("test@ksu.edu.sa", $mail, "baderm@ksu.edu.sa", $mailObject);
                            #$isSent = $this->sendPhishingMail2("test@ksu.edu.sa", $mail, $employee->email, $mailObject);
        ...
        $campaign = PhishingCampaign::withTrashed()->with([
            'employees.deliverdCampaigns' => function ($query) use ($id) {
                $query->where('campaign_id', $id)
                    ->with([
                        'deliverdEmailTemplates' => function ($query) {
    ...
    {
        $user = User::with([
            'campaigns.trainingModules' => function ($query) use ($id) {
    ...
    public function Certificates()
    {
        $breadcrumbs = [
            ['link' => route('admin.dashboard'), 'name' => __('locale.Dashboard')],
            ['link' => route('admin.phishing.dashboard'), 'name' => __('locale.Dashboard')],
            ['name' => __('locale.Certificates')]
        ];

        $certificates = LMSTrainingModuleCertificate::with('user')
            ->orderBy('issued_at', 'desc')
            ->get();

        return view('admin.content.phishing.campaign.certiificates', compact( 'certificates', 'breadcrumbs'));
    }

    public function listCertificatesAjax()
    {
        $certificates = LMSTrainingModuleCertificate::with(['user', 'training'])->orderBy('issued_at', 'desc');
        ...
        return DataTables::of($certificates)
            ...
            ->addColumn('actions', function ($certificate) {
                ...
                $downloadUrl = route(
                    'admin.phishing.campaign.download-certificate',
                    [$certificate->training->id, $certificate->user->id]
                );

                $viewUrl = route(
                    'admin.phishing.campaign.view-certificate',
                    [$certificate->training->id, $certificate->user->id]
                );
    ...
    /**
     * Download certificate - Similar to downloadCourseCertificate but for Training Modules
     */
    ...
    /**
     * Delete certificate - Similar to deleteCertificate but for Training Modules
     */
