...
class DocumentationsDataController extends Controller
{
    ...
    public function storePolicy(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'policy_name' => 'required|string|max:255|unique:center_policies,policy_name', // Ensure unique policy_name
        ...
        // Begin transaction for safe database operations
        DB::beginTransaction();
        try {
            // Create a new policy
            $policy = CenterPolicy::create([
                'policy_name' => $request->policy_name,
                'document_ids' => implode(',', $request->document_id), // Store as comma-separated string
            ]);
            // Loop through each document_id and insert into DocumentPolicy
            foreach ($request->document_id as $documentId) {

                // Create the DocumentPolicy
                $policyDoc = DocumentPolicy::create([
                    'policy_id' => $policy->id, // Reference to the newly created policy
                    'document_id' => $documentId, // Document ID from the request
                ]);

                // Get the most recent AuditDocumentPolicy for this document
                $lastAudit = AuditDocumentPolicy::where('document_id', $documentId)->latest()->first();

                if ($lastAudit) {
                    // If there's a last audit, insert into the `audit_document_policy_policy_document` table
    ...
    public function editPolicy(Request $request, $id)
    {
        ...
        $request->validate([
            'policy_name' => 'required|string|max:255|unique:center_policies,policy_name,' . $id, // Ensure unique except for the current ID
            'document_id' => 'required|array',
            'document_id.*' => 'exists:documents,id', // Validate each document ID
        ]);
    ...
    public function storePolicyDocument(Request $request)
    {
        ...
        $validatedData = $request->validate([
            ...
            'policy_id' => 'nullable|integer|exists:center_policies,id', // Allow policy_id to be nullable
            'new_policy_name' => 'nullable|string|max:255|unique:center_policies,policy_name', // Validate new policy name if provided
        ]);

        // If new policy name is provided, create a new policy
        if ($request->filled('new_policy_name')) {
            $policy = CenterPolicy::create([
                'policy_name' => $request->new_policy_name,
                'document_ids' => implode(',', (array)$request->document_id), // Handle document association
            ]);

            // Set the new policy ID to attach it to the document
            $validatedData['policy_id'] = $policy->id;
        } elseif ($request->filled('policy_id')) {
            // Find the existing policy by ID
            $policy = CenterPolicy::find($validatedData['policy_id']);
            if ($policy) {
                ...
                $mergedDocumentIds = array_unique(array_merge($existingDocumentIds, [(string)$validatedData['document_id']]));

                // Update the document_ids field with merged data
                $policy->update([
                    'document_ids' => implode(',', $mergedDocumentIds), // Save merged document_ids as a comma-separated string
                ]);
            }
        }
        ...
        if ($document) {
            ...
            $document->policies()->attach($validatedData['policy_id'], [
                'created_at' => now(), // Set the current timestamp
                'updated_at' => now(), // Set the updated timestamp if needed
            ]);
            ...
            return response()->json([
                'success' => true,
                'policy_name' => CenterPolicy::find($validatedData['policy_id'])->policy_name,
    ...
    public function ajaxGetList(Request $request)
    {
        ...
        foreach ($documents as $document) {
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-shield me-50 font-small-4">
    ...
    public function openImportForm()
    {
        ...
        $databaseColumns = [
            // Column: 'name'
            ['name' => 'policy_name', 'rules' => ['required'], 'example' => 'Policy Clause 1'],
            // Column: 'description'
