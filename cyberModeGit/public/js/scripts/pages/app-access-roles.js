$(function () {
    ...
    roleEdit.on('click', function () {
     ...
     const selectAll = document.querySelector('.selectAllpermissionGroup'),
 ...
 selectAll.addEventListener('change', t => {
    ...
    $('.modal').on('hidden.bs.modal', function() {
        ...
        $('input[name="name"]').val('');
        $("#addRoleForm").data('type', 'create');
        $("#addRoleForm").attr('action', storeRoute);
...
function ShowModalEditRolePermission(id) {

    url = getRoute.replace(':id', id);
    updateRouteUrl =updateRoute.replace(':id', id);
    $.ajax({
        ...
        , success: function (response) {
            if (response.status) {
                // Start Assign Role Permission  data to modal
                const editForm = $("#addRoleForm");
                editForm.data('type', 'update');
                editForm.attr('action', updateRouteUrl);
                $('input[name="name"]').val(response.data.name);
                response.data.permissions.forEach(permissionId => {
                    editForm.find('[value="'+ permissionId +'"]').attr('checked', true);
                });
                console.log(editForm.attr('action'));
                // End Assign Role Permission data to modal
          ...
          $.ajax({
            ...
            error: function (response) {
                if (response.status === 422) { // Validation Error
                    ...
                    makeAlert('error', errorMessage, "{{ __('locale.Validation Error') }}");
                } else {
                    makeAlert('error', response.responseJSON.message, "{{ __('locale.Error') }}");
                }
            }
        });
    });
...
function ShowModalDeleteRolePermission(id) {
    const deleteRouteUrl = deleteRoute.replace(':id', id);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            }, 0); // Adjust the delay time as needed (e.g., 100 milliseconds)
        },
        error: function (response) {
            // Reload the page after a short delay
            setTimeout(function () {
