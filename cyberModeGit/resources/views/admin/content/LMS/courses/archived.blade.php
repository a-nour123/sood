                        ...
                        <div class="col-sm-6 pe-0" style="text-align: end;">
                            <div class="action-content">
                                @if (auth()->user()->hasPermission('asset.create'))
                                    ...
                                    {{-- <a href="{{ route('admin.phishing.domains.index') }}"
                                        class=" btn btn-primary" title="Domains">
...
<div class="row" id="domains-parent-div">
    @foreach ($archived_domains as $index => $category)
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9 py-1">
                                ...
                                <button class="btn btn-secondary show-frame trash-domain" type="button" data-bs-toggle="modal"
                                        data-id="{{ $category->id }}" onclick="ShowModalRestoreDomain({{ $category->id }})" data-name="{{ $category->name }}">
...
<script>
    ...
    function TrashDomain(id) {
        ...
        let url = "{{ route('admin.phishing.domains.delete', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            url: url,
            type: "DELETE",
  ...
  function RestoreDomain(id) {
        let url = "{{ route('admin.phishing.domains.restore', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            url: url,
            type: "POST",
