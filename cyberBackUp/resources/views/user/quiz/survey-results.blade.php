    ...
    <div class="container-fluid statistics-container">
        ...
        <div class="stats-header fade-in">
            <h1><i class="fas fa-chart-line me-3"></i>Survey Statistics for course <span
                    class="text-info">{{ optional($trainingModule)->name }}</span></h1>
        ...
        @else
            ...
            <div class="no-data-message fade-in">
                ...
                <a href="{{ route('admin.lms.courses.index') }}" class="btn btn-primary">
    ...
    <script>
        $(document).ready(function() {
            var responsesTable = $('#surveyResponsesTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.lms.survey.ajax', ['type' => $type ?? '', 'id' => $id ?? '']) }}",
                    type: 'GET',
        ...
        function deleteSurveyResponse(responseId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{route('admin.lms.survey.response.delete', ['responseId' => '_responseId_'])}}".replace('_responseId_', responseId),
