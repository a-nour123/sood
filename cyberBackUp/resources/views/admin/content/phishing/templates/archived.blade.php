                    ...
                    <div class="col-sm-6 pe-0" style="text-align: end;">
                        <div class="action-content">
                            @if (auth()->user()->hasPermission('asset.create'))
                                ...
                                <a href="{{ route('admin.phishing.emailTemplate.index') }}"
            ...
            <div class="container-fluid product-wrapper">
                <div class="product-grid">
                    ...
                    <div class="product-wrapper-grid" id="website-parent-div">
                        <div class="row">
                            @foreach ($archived_email_templates as $email)
                            <div class="col-xl-3 col-sm-6 xl-4 website-card"  data-id="{{ $email->id }}">
                                <div class="card" >
                                    <div class="product-box">
                                        <div class="product-img">
                                            <img class="img-fluid" src="{{ asset( $email->attachment) }}" alt="">
                                            <div class="product-hover">
                                                <ul>
                                                    <li><a class=" show-frame trash-website" data-bs-toggle="modal"
                                                        data-id="{{ $email->id }}" onclick="ShowModalRestoreWebsite({{ $email->id }})" data-name="{{ $email->name }}">
                                                                                       <i class="fa-solid fa-undo"></i>

                                                    <li><a class="edit-regulator" data-bs-toggle="modal"
                                        data-id="{{ $email->id }}" onclick="ShowModalDeleteWebsite({{ $email->id }})"  data-name="{{ $email->name }}"><i class="fa-solid fa-trash"></i></a>
                                    </li>
...
<script>
    ...
    function updateQuillContent() {
        ...
        document.getElementById('supplemental_guidance_input').value = quillContent;
        console.log('Quill Content:', quillContent); // Debugging line

    }

    document.getElementById('add-new-website').addEventListener('submit', function(e) {
        updateQuillContent();
    });
...
<script>
    ...
    function restoreWebsite(id) {
        let url = "{{ route('admin.phishing.emailTemplate.restore', ':id') }}";
    ...
    function deleteWebsite(id) {
        let url = "{{ route('admin.phishing.emailTemplate.delete', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            url: url,
            type: "DELETE",
    ...
    $(document).ready(function() {
        ...
        $('#search-input').on('input', function() {
            ...
            timeout = setTimeout(function() {
                $.ajax({
                    url: "{{ route('admin.phishing.website.searchTrash') }}", // Update with your search route
                    type: "GET",
                    data: { query: query },
                    success: function(data) {
