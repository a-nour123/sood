                        ...
                        <div class="col-sm-6 pe-0" style="text-align: end;">
                            ...
                            <div class="action-content">
                                @if (auth()->user()->hasPermission('asset.create'))
                                    ...
                                    {{--  <a href="{{ route('admin.phishing.archivedCategories') }}"
                                    class=" btn btn-primary" target="_self">
    ...
    @foreach ($categories as $index => $category)
        <div class="col-4 domain-card" data-id="{{ $category->id }}">
            <div class="regulator-item p-3">
                @php
                $percentage = Helper::ImplementedStatisticWebsitesOfCategory($category->id);
                $color = '';
                        ...
                        <div class="row">
                            <div class="col-md-3 ">
                                <div class="chart-progress me-3" data-color="{{  $color }}"
                                     data-series="{{ Helper::ImplementedStatisticWebsitesOfCategory($category->id) }}" data-progress_variant="true"></div>
                            </div>
                            <div class="col-md-9 py-1">
                                ...
                                <a class="btn btn-secondary show-frame" href="{{ route('admin.phishing.getCategoryWebsites', $category->id) }}" title="Websites">
...
<x-phishing-category-form id="add-new-category" title="{{ __('locale.AddANewCategory') }}" />
{{--  @endif  --}}
<!--/ Create Form -->

<!-- Update Form -->
{{--  @if (auth()->user()->hasPermission('asset.update'))  --}}
<x-phishing-domains-form id="edit-regulator" title="{{ __('locale.EditWebsite') }}" />
{{--  @endif   --}}
<!--/ Update Form -->
...
<script>
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status) {
                    ...
                    location.reload();
                    // $('#domains-parent-div').append(data.newCategoryTemplate);
                } else {
    ...
    $('#edit-regulator form').submit(function(e) {
        ...
        let url = "{{ route('admin.phishing.phishingcategory.update', ':id') }}";
    ...
    function TrashCategory(id) {
        let url = "{{ route('admin.phishing.categories.trash', ':id') }}";
        url = url.replace(':id', id);
        $.ajax({
            url: url,
            type: "POST",
